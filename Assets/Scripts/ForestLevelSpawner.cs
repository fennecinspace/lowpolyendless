...
public class ForestLevelSpawner : MonoBehaviour {
    ...
    bool VerifyEmptySpace(Vector3 pos) { // takes pos for instantiation 
        pos.y += 20; // adds to y pos to raycast from above the pos
        return Physics.Raycast(pos, Vector3.down); // raycasting then returning -> true for "collider found" ## -> false for "no collider found"  
    }
